...
var JobNode = _Base.Class.define(function (id, work, priority, context, name, asyncOpID) {
    ...
    _setPriority: function (value) {
        if (+this._priority === this._priority && this._priority !== value) {
            jobProfilerMark(this, "job-priority-changed", "info", markerFromPriority(this._priority).name, markerFromPriority(value).name);
        }
...
var OwnerToken = _Base.Class.define(function OwnerToken_ctor() {
    ...
    cancelAll: function OwnerToken_cancelAll() {
        ...
        /// </summary>
        /// </signature>
        var jobs = this._jobs, jobIds = Object.keys(jobs);
        this._jobs = {};
...
var priorities = [
    ...
    new MarkerNode(-7, "-7"),
    new MarkerNode(-8, "-8"),
    new MarkerNode(-9, "belowNormal"),
    new MarkerNode(-10, "-10"),
    new MarkerNode(-11, "-11"),
    new MarkerNode(-12, "-12"),
    new MarkerNode(-13, "idle"),
    new MarkerNode(-14, "-14"),
    new MarkerNode(-15, "min"),
...
function toWwaPriority(winjsPriority) {
    if (winjsPriority >= Priority.aboveNormal + 1) {
        return MSApp.HIGH;
    }
    if (winjsPriority >= Priority.belowNormal) {
        return MSApp.NORMAL;
    }
    return MSApp.IDLE;
}
...
function run(scheduled) {
    ...
    try {
        ...
        var shouldYield = function () {
            ...
            if (wwaTaskScheduledAtPriorityHigherThan(toWwaPriority(highWaterMark))) {
                return true;
            }
            if (!!drainQueue.length) {
                return false;
...
_Base.Namespace._moduleDefine(undefined || _Global, "WinJS.Utilities.Scheduler", _schedulerExports);
